...
```text
Read AGENTS.md first. Improve CaseOh90000Panel.py UI clarity without changing mechanics. Keep tabs compact, avoid excessive inline text, keep path hiding on by default, and keep caseOh mOde described only as Easter Egg. Run the basic checks.
```

## 4. Release Build Check

```text
Read AGENTS.md first. Verify release hygiene. Confirm no game files, saves, copied branch folders, __pycache__, or pyc files are included. Update README/CHANGELOG/release notes if needed. Do not change mechanics.
```
