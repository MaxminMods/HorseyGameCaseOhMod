...
def panel_python_executable() -> str:
...
def open_panel_from_config(detached: bool = False) -> int:
    cfg = load_config()
    branch = Path(cfg["branch"])
    cmd = [panel_python_executable() if detached else sys.executable, str(tool_dir() / "CaseOh90000Panel.py"), "--branch", str(branch)]
    print("\n> " + " ".join(f'"{c}"' if " " in c else c for c in cmd))
    if detached:
        kwargs: Dict[str, Any] = {"cwd": str(tool_dir())}
        flags = detached_panel_flags()
        if flags:
...
def cmd_panel(args: argparse.Namespace) -> int:
    return open_panel_from_config(detached=True)
